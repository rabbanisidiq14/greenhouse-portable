            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('topics.update', $topics->id) }}" method="POST">
                  ...
                  <div class="mb-4">
                      ...
                      <input type="text" id="topic_name" name="topic_name" value="{{ $topics->topic_name }}" class="mt-1 block w-full bg-gray-800 text-white border border-gray-600 rounded-md shadow-sm focus:ring focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div class="mb-4">
                      ...
                      <select name="client_id" id="clientSelect" class="block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:focus:ring-indigo-500 dark:focus:border-indigo-500">
                          @foreach($clients as $client)
                              <option value="{{ $client->id }}" {{ $client->id == $topics->client_id ? 'selected' : '' }}>{{ $client->client_id }} | {{ $client->client_name }}</option>
                          @endforeach
                      </select>
                  </div>
                  </div>
